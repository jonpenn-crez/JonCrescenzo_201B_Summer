...
struct AlloApp : App {
  ...
  //change  to -1 to 1 range and add to GUI
  //testing if I can attraction and repulusion
  Parameter dragFactor{"/dragFactor", "", 0.1, 0, 0.9};
  //
  ...
  void onAnimate(double dt) override {
    ...
    // get the position of the particle
      ...
      force[i] += springForce * 4; // add the spring force to the total force on this particle   
      ...
      for (int j = i + 1; j < mesh.vertices().size(); ++j) {
        if (i == j){ 
          continue; 
            ...
            //adds the dragfactor to the opposite velocity 

            force[i] += -velocity[i] * dragFactor  ; // F = -bv
            force[j] += -velocity[j] * dragFactor ; // F = -bv
            
            //normalized the vector or the points shoot off into infinity, nan.
            // normalized to 0.5 so they don't go too fast

            //  if(velocity[i].mag() > 0.5 && velocity[j].mag()>0.5)  { 
           
            if(velocity[j].mag() > 0.5 && velocity[j].mag()>0.5)  {
            
           
            //rotation around orgin 
            velocity[i] = velocity[i].normalize(0.5);
             velocity[j] = velocity[j].normalize(0.5);

           

            }
            //normalizing out side the if statement
              //  velocity[j] = velocity[j].normalize(0.5);

            // if(velcoity[j].mag() > 0.5) {
            //   velocity[j] = velocity[j].normalize(0.5);
            // } 
          ...
          auto& me = mesh.vertices()[i];
          auto& them = mesh.vertices()[j];

          Vec3f difference = me - them;
          float distance = difference.mag();

          if (distance < 0.5) {
            // F = k * (1/distance^2)
            Vec3f repulsionForce = difference.normalize(1 / (distance * distance));
            force[i] += repulsionForce;
            force[j] -= repulsionForce; // equal and opposite
          }
