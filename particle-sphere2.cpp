...
struct AlloApp : App {
  Parameter pointSize{"/pointSize", "", 5.0, 1.0, 20.0};
  ...
  void onCreate() override {
    ...
    auto randomSize = []() { return rnd::uniform(1.0, 20.0); };
    // printf("random size: %f\n", randomSize());
    ...
    for (int _ = 0; _ < 1000; _++) {
      ...
      //call the textcoord to set the size of the point
      mesh.texCoord(randomSize(), 0);  // s, t  
      ...
      bool isPositive = mesh.vertices().back().x > 0 || mesh.vertices().back().y > 0 || mesh.vertices().back().z > 0; // check if the particle is in the positive or negative region of space
      if (isPositive) {
        mesh.colors().back() = RGB(1, 0, 0); // red for positive region
      } else {
        mesh.colors().back() = RGB(0, 0, 1); // blue for negative region
      }
    }
  ...
  void onAnimate(double dt) override {
    ...
    for (int i = 0; i < velocity.size(); i++) {
     ...
     if(distance < 0.5) {
      //  printf("too close to the origin! distance: %f\n", distance);
      //change color to red if too close to the origin
      mesh.colors()[i] = RGB(0, 0, 0);
     }
...
///////// you probably do not need to edit the rest of this method...
  ...
  //Mouse interaction to add more particles...doesnt work
