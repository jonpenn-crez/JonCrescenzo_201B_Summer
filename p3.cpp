...
struct MyApp : public App {
  ...
  void onCreate() override {
    ...
    mesh2.scale(1, 0.2, 1);   // flatten it (make it a narrow cone)
    mesh2.scale(1);         // make it small overall
  ...
  void onAnimate(double dt) override {
    ...
    if (F != lastN) {
      lastN = F;
      resetFood(F);  // re-initialize food if F slider changed
    }
