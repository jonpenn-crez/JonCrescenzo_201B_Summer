...
struct AlloApp : DistributedAppWithState<WorldState> {
  ...
  void loadCSV(string filename) {
    ...
    while (getline(file, line)) {
      if (words.size() >= MAX_POINTS) break;
  ...
  Color categoryColor(string category) {
    if (category == "Intercept") return Color(1, 1, 1, 0.5);
    if (category == "NYT") return Color(0.5, 0.5, 0.25, 0.5);
    if (category == "WSJ") return Color(0.75, 0.75, 1, 0.5);
    if (category == "FTTimes") return Color(0.5, 0.5, 1, 0.5);
    if (category == "DropSite") return Color(1, 0.5, 0.5, 0.5);

    return Color(1, 1, 1, 0.25);
  }

  void buildLineMesh(vector<Vec3f> &position) {
    lineMesh.reset();
    lineMesh.primitive(Mesh::LINES);

    if (state().timer <= 8.0) return;

    float t = (state().timer - 8.0) / 3.0;
    if (t < 0) t = 0;
    if (t > 1) t = 1;
    ...
    for (int i = 0; i < position.size(); i++) {
      for (int j = i + 1; j < position.size(); j++) {
        if (categories[i] == categories[j]) {
          ...
          Color inlineColor = categoryColor(categories[i]);
  ...
  void onAnimate(double dt) override {
    ...
    if (isPrimary()) {
      ...
      for (int i = 0; i < position.size() && i < MAX_POINTS; i++) {
      ...
      for (int i = 0; i < position.size() && i < state().numPoints; i++) {
        position[i] = state().positions[i];
      }
    }

    buildLineMesh(position);
  }

  void drawAnimatedOutline(Graphics &g) {
    if (state().timer <= 2.5) return;

    Mesh outline;
    outline.primitive(Mesh::LINES);

    float w = boxWidth;
    float h = boxHeight;

    float t = (state().timer - 5.5) / 1.0;
    if (t < 0) t = 0;
    if (t > 1) t = 1;
