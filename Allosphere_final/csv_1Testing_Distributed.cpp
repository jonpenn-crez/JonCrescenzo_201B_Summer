...
struct AlloApp : DistributedAppWithState<WorldState> {
  ...
  void onCreate() override {
    ...
    loadCSV("/Users/joncrescenzo/allolib_playground/JonCrescenzo_201B_Summer/Allosphere_final/test5.csv");
      ...
      for (int i = 0; i < position.size(); i++) {
        for (int j = i + 1; j < position.size(); j++) {
          if (categories[i] == categories[j]) {
            ...
            if (categories[i] == "WSJ") {
              inlineColor = Color(1, 0.5, 0.5, alphaLines);
            }

            if (categories[i] == "FTTimes") {
              inlineColor = Color(1.0, 0.694, 0.078, alphaLines);
            }
